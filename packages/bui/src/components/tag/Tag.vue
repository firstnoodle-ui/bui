...
<template>
  <!-- <BFlexbox
    inline
    class="h-8 gap-0 truncate text-sm border rounded-lg border-default text-secondary bg-secondary"
    :class="{
      'px-2': !deletable,
      'pl-2 pr-1': deletable,
    }"
  >
    ...
    <BButton v-if="deletable" small variant="textSubtle" icon="close" class="ml-1" stop-propagation @click="emit('delete')" />
  </BFlexbox> -->
  <BFlexbox
    inline
