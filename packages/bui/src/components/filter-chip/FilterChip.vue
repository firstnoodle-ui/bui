...
<template>
  <button
    ...
    :class="{
      ...
      'pl-2 pr-1 border border-transparent text-white bg-cyan-600 hover:bg-cyan-700 active:bg-cyan-800': active,
    }"
    @click="$emit('click')"
  >
    ...
    <BDeleteButton v-if="active" data-theme="dark" @click="$emit('delete')" />
  </button>
</template>
