...
import type { SelectListOption, SelectListFilter, SelectListProps, SelectListState, SelectListOptionGroup } from './types.ts';
...
import { BButton, BCheckbox, BFilterChip, BInput, BScrollbar } from "../..";
...
const localOptions = ref<SelectListOption[]|SelectListOptionGroup<T>[]>([])
...
const isOptionGroup = (obj: unknown): obj is SelectListOptionGroup<any> => {
  return (
    typeof obj === 'object' &&
    obj !== null &&
    'options' in obj &&
...
const grouped = computed(() => isOptionGroup(props.options));
...
const updateLocalOptions = (options:T[]|SelectListOptionGroup<T>[]) => {
...
<template>
    ...
    >
        ...
        >
            ...
            <section v-if="filters" class="flex items-center gap-2 px-4 last:pb-4">
                <BFilterChip
                    ...
                    :key="filter.name"
                    :active="Boolean(selectedOptionFilter && selectedOptionFilter.name === filter.name)"
                    :label="filter.name"
            ...
            <button
                v-if="selectAll"
                class="flex items-center gap-2 px-4 py-1 mb-1 rounded-lg hover:bg-secondary active:bg-tertiary"
                @click="onToggleAll"
            > 
                <BCheckbox
        ...
        <main class="flex-1 h-full overflow-hidden flex flex-col">
            <BErrorCard v-if="state === 'error'" :message="<string>errorMessage" />
