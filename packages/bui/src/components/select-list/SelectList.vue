<template>
    ...
    >
        <header
            ...
            class="flex-0 flex flex-col border-b-1 border-outline-subtle"
        >
            <slot name="header" />

            <section v-if="searchFunction" class="px-4 pb-2">
                <NSearchField
                    ref="searchFieldRef"
            ...
            <section v-if="filters" class="flex items-center gap-2 px-4 last:pb-4">
                <SelectListFilterChip
                    v-for="filter in filters"
                    :key="filter.name"
                    :selected="Boolean(selectedOptionFilter && selectedOptionFilter.name === filter.name)"
                    :label="filter.name"
                    @click="onOptionFilterChange(filter)"
                />
            </section>

            <section v-if="selectAll" class="flex items-center gap-2 px-4"> <!-- border-t-1 border-outline-subtle -->
                <NCheckbox
                    :indeterminate="Boolean(selectedCount && !allOptionsSelected)"
                    :model-value="allOptionsSelected"
                    @update:model-value="onToggleAll"
                />
                <span class="text-ui-label-14-regular text-disabled-font italic">Select/deselect all</span>
            </section>
        </header>

        <main class="flex-1 h-full overflow-hidden flex flex-col">
            <ErrorCard v-if="state === 'error'" :message="<string>errorMessage" />
            <LoadingCard v-else-if="state === 'loading'" />
            <NoMatchCard v-else-if="state === 'no match'" />
            <IdleCard v-else-if="state === 'idle'" />
            <Scrollbar
            ...
            >
                <ul class="py-2" ref="optionsRef">
                    <slot
                        ...
                        :select="onSelect"
                        :getIsSelected="optionIsSelected"
                    />
                </ul>
            </Scrollbar>
        </main>
        ...
        <template v-if="$slots.footer">
            <footer class="flex-initial max-h-72 flex flex-col border-t-1 border-outline-subtle">
                <template v-if="showSelection">
                    <header class="flex-none flex items-center justify-between gap-1 px-4 py-1">
                        <NButton
                            ...
                            :trailing-icon="selectionOpen ? 'chevronDown' : 'chevronUp'"
...
// import { NButton, NCheckbox, NInputChip, NSearchField } from "@novonordisk/novoui-vuejs";
import debounce from "debounce";
import { scrollIntoView } from '@/utils/scrollIntoView';
import { useNextFrame } from '@/composables/useNextFrame';
import { Scrollbar } from '@/components';
import SelectListFilterChip from './SelectListFilterChip.vue';
import { SelectListOption, SelectListFilter, SelectListProps, SelectListState } from './types.ts';
import { ErrorCard, IdleCard, LoadingCard, NoMatchCard } from './StatusCards/index.ts';
import { ScrollIntersectionEvent } from '@/components/BaseComponents/Scrollbar/types';
...
const scrollbarRef = ref<typeof Scrollbar>()
const selectedOptionsRef = ref()
const selectionScrollbarRef = ref<typeof Scrollbar>()
...
const focusSearch = () => searchFieldRef.value && searchFieldRef.value.inputExposed.focus();
...
const scrollToOption = (option:SelectListOption) => {
    console.log(optionsRef.value.children)
...
defineExpose({ focusSearch, multiselect, reset, scrollbarRef });
</script>